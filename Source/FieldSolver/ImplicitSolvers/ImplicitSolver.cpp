    ...
    const bool a_zero_out_first )
{
    ...
    AMREX_ALWAYS_ASSERT(a_in.size() == static_cast<std::size_t>(nlevs));
    if (use_delta) {
        AMREX_ALWAYS_ASSERT(a_in_ref->size() == static_cast<std::size_t>(nlevs));
    }
    if (use_baseline) {
        AMREX_ALWAYS_ASSERT(a_baseline->size() == static_cast<std::size_t>(nlevs));
    }
...
#pragma omp parallel if (amrex::Gpu::notInLaunchRegion())
        ...
        {
            amrex::Array4<amrex::Real> const& Ox = a_out[lev][0]->array(mfi);
            amrex::Array4<amrex::Real> const& Oy = a_out[lev][1]->array(mfi);
            amrex::Array4<amrex::Real> const& Oz = a_out[lev][2]->array(mfi);

            amrex::Array4<const amrex::Real> const& Ix = a_in[lev][0]->array(mfi);
            amrex::Array4<const amrex::Real> const& Iy = a_in[lev][1]->array(mfi);
            amrex::Array4<const amrex::Real> const& Iz = a_in[lev][2]->array(mfi);

            amrex::Array4<const amrex::Real> const& Ix0 = use_delta ? (*a_in_ref)[lev][0]->array(mfi) : Ix;
            amrex::Array4<const amrex::Real> const& Iy0 = use_delta ? (*a_in_ref)[lev][1]->array(mfi) : Iy;
            amrex::Array4<const amrex::Real> const& Iz0 = use_delta ? (*a_in_ref)[lev][2]->array(mfi) : Iz;

            amrex::Array4<const amrex::Real> const& Bx = use_baseline ? (*a_baseline)[lev][0]->array(mfi) : Ox;
            amrex::Array4<const amrex::Real> const& By = use_baseline ? (*a_baseline)[lev][1]->array(mfi) : Oy;
            amrex::Array4<const amrex::Real> const& Bz = use_baseline ? (*a_baseline)[lev][2]->array(mfi) : Oz;
                ...
                for (int ii = index_min[0]; ii <= index_max[0]; ++ii) {
                    for (int jj = index_min[1]; jj <= index_max[1]; ++jj) {
                        for (int kk = index_min[2]; kk <= index_max[2]; ++kk) {
                            ...
                            amrex::Real dval = Ix(i+ii,j+jj,k+kk,n);
                            if (use_delta) { dval -= Ix0(i+ii,j+jj,k+kk,n); }
                            SxxdIx += Sxx(i,j,k,Nc)*dval;
                        }
                    }
                }
                ...
                for (int ii = index_min[0]; ii <= index_max[0]; ++ii) {
                    for (int jj = index_min[1]; jj <= index_max[1]; ++jj) {
                        for (int kk = index_min[2]; kk <= index_max[2]; ++kk) {
                            ...
                            amrex::Real dval = Iy(i+ii,j+jj,k+kk,n);
                            if (use_delta) { dval -= Iy0(i+ii,j+jj,k+kk,n); }
                            SxydIy += Sxy(i,j,k,Nc)*dval;
                        }
                    }
                }
                ...
                for (int ii = index_min[0]; ii <= index_max[0]; ++ii) {
                    for (int jj = index_min[1]; jj <= index_max[1]; ++jj) {
                        for (int kk = index_min[2]; kk <= index_max[2]; ++kk) {
                            ...
                            amrex::Real dval = Iz(i+ii,j+jj,k+kk,n);
                            if (use_delta) { dval -= Iz0(i+ii,j+jj,k+kk,n); }
                            SxzdIz += Sxz(i,j,k,Nc)*dval;
                        }
                    }
                }

                if (use_baseline) { Ox(i,j,k,n) += Bx(i,j,k,n); }
                Ox(i,j,k,n) += a_scale * (SxxdIx + SxydIy + SxzdIz);
            });
            amrex::ParallelFor(
                ...
                for (int ii = index_min[0]; ii <= index_max[0]; ++ii) {
                    for (int jj = index_min[1]; jj <= index_max[1]; ++jj) {
                        for (int kk = index_min[2]; kk <= index_max[2]; ++kk) {
                            ...
                            amrex::Real dval = Iz(i+ii,j+jj,k+kk,n);
                            if (use_delta) { dval -= Iz0(i+ii,j+jj,k+kk,n); }
                            SyzdIz += Syz(i,j,k,Nc)*dval;
                        }
                    }
                }

                if (use_baseline) { Oy(i,j,k,n) += By(i,j,k,n); }
                Oy(i,j,k,n) += a_scale * (SyxdIx + SyydIy + SyzdIz);
            });
            amrex::ParallelFor(
                ...
                for (int ii = index_min[0]; ii <= index_max[0]; ++ii) {
                    for (int jj = index_min[1]; jj <= index_max[1]; ++jj) {
                        for (int kk = index_min[2]; kk <= index_max[2]; ++kk) {
                            ...
                            amrex::Real dval = Iz(i+ii,j+jj,k+kk,n);
                            if (use_delta) { dval -= Iz0(i+ii,j+jj,k+kk,n); }
                            SzzdIz += Szz(i,j,k,Nc)*dval;
                        }
                    }
                }

                if (use_baseline) { Oz(i,j,k,n) += Bz(i,j,k,n); }
                Oz(i,j,k,n) += a_scale * (SzxdIx + SzydIy + SzzdIz);
            });
        }
    }
}
...
void ImplicitSolver::ComputeJfromMassMatrices (const bool  a_J_from_MM_only)
{
    ...
    ApplyMassMatrices(J_ml, E_ml, &E0_ml, &J0_ml, 1.0_rt, a_J_from_MM_only);
}
