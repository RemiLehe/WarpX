                                      ...
                                      [[maybe_unused]] bool a_from_jacobian )
{
    ...
    // periodically wrapped cells agree before ApplyMassMatrices reads dA_fp below.
    for (int ii = 0; ii < 3; ii++)
    {
        dA_fp[lev][ii]->FillBoundaryAndSync(m_WarpX->Geom(lev).periodicity());
        // clear E_temp since ApplyMassMatrices accumulates into its rhs argument
        E_temp[lev][ii]->setVal(0);
    }
    // Calculate chi dA and write into E_temp
    ApplyMassMatrices(E_temp, dA_fp, nullptr, nullptr, 2._rt * PhysConst::mu0 / m_dt);
